...
from utils.crew_ai_integration import BICrewAI

class BIAgent:
    """
    Business Intelligence Agent powered by Gemini AI and CrewAI
    Handles data analysis, insights generation, and Q&A functionality
    """
    
    def __init__(self, api_key: str):
        """Initialize the BI Agent with Gemini API and CrewAI"""
        genai.configure(api_key=api_key)
        self.model = genai.GenerativeModel("gemini-1.5-flash-latest")
        self.api_key = api_key
        
        # Initialize CrewAI for advanced analysis
        try:
            self.crew = BICrewAI(api_key)
    ...
    def extract_data_from_document(self, content, filename: str) -> Optional[str]:
        ...
        try:
            if filename.endswith('.pdf'):
                # For PDF processing - simplified approach
    ...
    def generate_comprehensive_analysis(self, df: pd.DataFrame, goals: List[str]) -> str:
        """Generate comprehensive business analysis using CrewAI or fallback to Gemini"""
        
        # Try CrewAI first for more sophisticated analysis
        if self.crew_available:
            try:
                return self.crew.analyze_dataset(df, goals)
            except Exception as e:
                print(f"CrewAI analysis failed, falling back to Gemini: {e}")
    ...
    def answer_question(self, df: pd.DataFrame, question: str) -> str:
        """Answer specific questions about the data using CrewAI or Gemini"""
        
        # Try CrewAI for business questions
        if self.crew_available:
            try:
                return self.crew.answer_business_question(df, question)
            except Exception as e:
                print(f"CrewAI Q&A failed, falling back to Gemini: {e}")
    ...
    def generate_financial_deep_dive(self, df: pd.DataFrame) -> str:
        ...
        # Try CrewAI financial analysis first
        if self.crew_available:
            try:
                return self.crew.financial_deep_dive(df)
            except Exception as e:
                print(f"CrewAI financial analysis failed: {e}")
        
        # Fallback to Gemini financial analysis
        try:
            data_summary = self._get_data_summary(df)
            financial_metrics = self._calculate_financial_metrics(df)
